...
public class AdminHospital {
	...
	public static void findHospitalsByAdminPhonePassword() {
		...
		long phone = s.nextLong();
		String password = s.next();
		List<Hospital> hospitals = hospitalDao.findHospitalsByAdminEmailPassword(phone, password);
		if (hospitals.size() > 0) {
	...
	public static void findHospitalsByAdminEmailPassword() {
		...
		String email = s.next();
		String password = s.next();
		List<Hospital> hospitals = hospitalDao.findHospitalsByAdminPhonePassword(email, password);
		if (hospitals.size() > 0) {
			for (Hospital h : hospitals) {
				System.out.println("Hospital Id:" + h.getId());
				System.out.println("Hospital Name:" + h.getName());
